module EtsyApi
  ...
  class SecureClient
    ...
    def consumer # :nodoc:
      path = '/v2/oauth/'
      @consumer ||= OAuth::Consumer.new(EtsyApi.api_key, EtsyApi.api_secret, {
        :site               => "#{EtsyApi.protocol}://#{EtsyApi.host}",
        :request_token_path => "#{path}request_token?scope=#{EtsyApi.permission_scopes.join('+')}",
    ...
    #
    def request_token
      consumer.get_request_token(:oauth_callback => EtsyApi.callback_url)
    end
    ...
    def post_multipart(endpoint, params = {})
      client = Net::HTTP.new(EtsyApi.host, EtsyApi.protocol == "http" ? 80 : 443)
      client.use_ssl = true if EtsyApi.protocol == "https"
